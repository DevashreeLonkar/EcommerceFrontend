...
export class ProductService {
  ...
  public getAllProducts(pageNumber: number, searchKey: string=""){
    return this.httpClient.get<Product[]>("http://localhost:8080/getAllProducts?pageNumber="+pageNumber+"&searchKey="+searchKey);
  }
  ...
  public getProductDetails(isSingleProductCheckout: boolean, productId: string){
  ...
  public getAllOrderDetailsForAdmin(orderStatus: String): Observable<MyOrderDetails[]> {
    return this.httpClient.get<MyOrderDetails[]>("http://localhost:8080/getAllOrderDetails/"+orderStatus);
  }
